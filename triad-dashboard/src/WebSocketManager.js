class WebSocketManager {
  ...
  updateHealthIndicator(data) {
    ...
    if (indicator) {
      indicator.className = `health-indicator ${data.color}`;
      indicator.setAttribute('title', `System Status: ${data.status}`);
    }
  }

  // Update cost display
  updateCostDisplay(data) {
    const costDisplay = document.querySelector('.cost-tracker');
    if (costDisplay) {
      costDisplay.textContent = `$${data.current.toFixed(2)} / $${data.total.toFixed(2)}`;
    }
  }

  // Handle killswitch activation
  handleKillswitch(data) {
    const killswitch = document.querySelector('.killswitch-toggle');
    if (killswitch) {
      killswitch.classList.toggle('active', data.active);
      if (data.active) {
        killswitch.setAttribute('title', `Killswitch Active: ${data.reason}`);
      }
    }
  }
